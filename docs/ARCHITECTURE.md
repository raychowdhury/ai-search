...
| Google Gemini API (adapter built 2026-09-21, unverified live) | Interactions endpoint `POST /v1beta/interactions` with `tools: [{type: "google_search"}]`; default `gemini-2.5-flash` | No location parameter; the city is appended to the question text | Yes: `url_citation` annotations with url and title | Free tier: 2.5 models with grounding up to 500 requests/day (prompts may be used by Google); paid 2.5: $35 per 1,000 grounded prompts plus tokens; 3.x grounding $14 per 1,000 (not free) | `GEMINI_API_KEY` |
