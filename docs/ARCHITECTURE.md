...
| Google Gemini API (adapter built 2026-09-21; grounding needs a billed project) | Interactions endpoint `POST /v1beta/interactions` with `tools: [{type: "google_search"}]`; default `gemini-3.6-flash` | No location parameter; the city is appended to the question text | Yes: `url_citation` annotations with url and title | New free-tier accounts get no grounding quota on 3.x and no access to 2.5; on a billed project 3.x grounding is 5,000 free requests/month then $14 per 1,000, plus tokens. Free tier still works for extraction. | `GEMINI_API_KEY` |
...
Mention and recommendation extraction uses deterministic name matching for the owner's business (name and aliases, normalized) plus an LLM extraction step. The extractor is chosen in this order from configured keys: Claude (structured outputs), OpenAI (JSON in text, verified live 2026-09-21), Gemini (JSON in text, unverified); `EXTRACTOR` forces one. Each run's fingerprint records which model extracted. The extractor that lists businesses named in the answer with verbatim evidence excerpts. Every excerpt is verified as a substring of the stored answer before it is saved; excerpts that do not match are discarded and logged. If no analysis key is configured, only name matching runs and the UI says "competitor extraction unavailable".
